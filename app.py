...
# Select relevant columns from the product data
...
# Fill missing values in 'tertiary_category' with values from 'secondary_category'
...
# Select relevant columns from the reviews data
df_review = df_reviews[['author_id', 'product_id', 'rating']]

# Remove duplicate rows based on 'author_id' and 'product_id'
df_review = df_review.drop_duplicates()

# Create a reader object for collaborative filtering
reader = Reader(rating_scale=(0, 5))

# Load review data into a Surprise Dataset
...
# Build a full training set for collaborative filtering
...
if choice == 'FavFinder':
    ...
    if st.button("Get squad suggestions"):
        ...
        recs = get_user_recommendations(user_input, knn_best_model, df_review, df_product_info, n=preference)
